...
BEGIN
  ...
  IF public.is_jae_member(NEW.user_id) THEN
    ...
    PERFORM public.recalculate_academy_profile(NEW.user_id, true);
  ELSE
    ...
    PERFORM public.recalculate_academy_profile(NEW.user_id, false);
  END IF;
...
WITH best_transcript AS (
  SELECT
    ct.user_id,
    ct.module_id,
    c.day_id,
    greatest(0, least(10, max(ct.score))) AS best_score,
    max(ct.created_at) AS completed_at
  FROM public.coaching_transcripts ct
  JOIN public.academy_module_catalog c
    ON c.module_id = ct.module_id
   AND c.module_type = 'coach-chat'
   AND c.is_active
  WHERE ct.score IS NOT NULL
  GROUP BY ct.user_id, ct.module_id, c.day_id
), main_best AS (
  SELECT * FROM best_transcript WHERE NOT public.is_jae_member(user_id)
), jae_best AS (
  SELECT * FROM best_transcript WHERE public.is_jae_member(user_id)
)
INSERT INTO public.user_progress (user_id, day_id, module_id, status, score, completed_at)
SELECT user_id, day_id, module_id, 'completed', (best_score * 1000) + 10, completed_at
FROM main_best
ON CONFLICT (user_id, module_id) DO UPDATE SET
  day_id = EXCLUDED.day_id,
  status = 'completed',
...
BEGIN
  FOR u IN SELECT DISTINCT user_id FROM public.user_progress LOOP
    PERFORM public.recalculate_academy_profile(u.user_id, false);
  END LOOP;

  FOR u IN SELECT DISTINCT user_id FROM public.jae_user_progress LOOP
    PERFORM public.recalculate_academy_profile(u.user_id, true);
  END LOOP;
END $$;
